...
// Session Driver Commands (async)
//
// These commands provide the new session-based API for pi-gui.
// They use the async Mutex to ensure the UI remains responsive.
...
#[tauri::command]
pub async fn pi_archive_session(
    _session_ref: Value,
) -> Result<(), String> {
    // TODO: Implement full session driver
    Ok(())
...
#[tauri::command]
pub async fn pi_unarchive_session(
    _session_ref: Value,
) -> Result<(), String> {
    // TODO: Implement full session driver
    Ok(())
...
pub async fn pi_send_user_message(
    ...
    // For now, route through legacy prompt command
    // TODO: Implement proper session driver routing
    let message = input.get("text")
        .and_then(|v| v.as_str())
        .unwrap_or("")
        .to_string();
...
pub async fn pi_set_session_model(
    ...
    let provider = selection.get("provider")
        .and_then(|v| v.as_str())
        .unwrap_or("")
        .to_string();
    let model_id = selection.get("modelId")
        .and_then(|v| v.as_str())
        .unwrap_or("")
        .to_string();
...
#[tauri::command]
pub async fn pi_rename_session(
    _session_ref: Value,
    _title: String,
) -> Result<(), String> {
    // TODO: Implement full session driver
...
#[tauri::command]
pub async fn pi_reload_session(
    _session_ref: Value,
) -> Result<(), String> {
    // TODO: Implement full session driver
    Ok(())
...
pub async fn pi_respond_to_host_ui_request(
    ...
    let id = response.get("requestId")
        .and_then(|v| v.as_str())
        .unwrap_or("")
        .to_string();
    let value = response.get("value").and_then(|v| v.as_str()).map(|s| s.to_string());
