...
/**
 * 测试天气查询扩展
 * 运行: node test-weather.ts
 */

import type { WeatherParamsType } from './weather-query';

// 模拟 fetch 函数
async function mockFetch(url: string): Promise<{ ok: boolean; text: () => Promise<string> }> {
...
async function testWeatherQuery() {
  ...
  const briefParams: WeatherParamsType = {
    city: 'Beijing',
    brief: true,
    forecast: false
  };
